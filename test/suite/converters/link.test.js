...
const linkConverter_1 = require("../../../src/converters/linkConverter");
suite("LinkConverter", () => {
    suite("convertLink", () => {
        test("[テキスト](https://example.com)を変換する", () => {
            const input = "[リンクテキスト](https://example.com)";
            const expected = "[[jumpuri:リンクテキスト > https://example.com]]";
            assert.strictEqual((0, linkConverter_1.convertLink)(input), expected);
        });
        test("http://も正しく変換する", () => {
        ...
        test("画像タグは変換しない", () => {
            ...
            assert.strictEqual((0, linkConverter_1.convertLink)(input), input);
        });
        test("複数のリンクを変換する", () => {
            const input = "[リンク1](https://a.com)と[リンク2](https://b.com)";
            const expected = "[[jumpuri:リンク1 > https://a.com]]と[[jumpuri:リンク2 > https://b.com]]";
            assert.strictEqual((0, linkConverter_1.convertLink)(input), expected);
        });
    });
});
